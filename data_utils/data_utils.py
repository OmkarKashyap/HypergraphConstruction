...
class SentenceDataset(Dataset):
    ...
    def __init__(self, fname, tokenizer, args, config, vocab_help, embedding_matrix, save_path):
        ...
        self.positional_encoding = PositionalEncoding(d_model=embedding_matrix.shape[1], max_len=args.max_length)
        self.multihead_attn = nn.MultiheadAttention(embed_dim=embedding_matrix.shape[1], num_heads=5, batch_first=True)
        ...
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
            ...
            attn_mask = torch.tensor(word_mask, dtype=torch.bool).unsqueeze(0).to(torch.float32) # Shape: (1, max_length)
            ...
            x, attn_output_weights = self.multihead_attn(query=aspect_embedding, key=x, value=x, key_padding_mask=attn_mask)
